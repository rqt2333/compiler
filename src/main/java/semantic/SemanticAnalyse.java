...
public class SemanticAnalyse {
    ...
    public static void call(String Action, int TokenIndex, Word word, SymbolTable table, LiveStatus LiveStatus) {
        switch (Action) {
            case Constants.PUSH: { // PUSH动作
                Pair<Word, Integer> _pair = new Pair<>(word, TokenIndex);
                Stack<Pair<Word, Integer>> sem = LiveStatus.getSEM();
                LiveStatus.getSEM().push(_pair);
                break;
            }

            case Constants.PUSHARG: { // PUSHARG动作
                Pair<Word, Integer> _pair = new Pair<>(word, TokenIndex);
                LiveStatus.getArgsStack().push(_pair);
                break;
            }
            case Constants.GEQA: { // GEQA动作
                Word str1 = LiveStatus.getSEM().peek().getFirst();
                LiveStatus.getSEM().pop();
                Word str2 = LiveStatus.getSEM().peek().getFirst();
                LiveStatus.getSEM().pop();
            ...
            case Constants.ASSI: { // ASSI动作
                Stack<Pair<Word, Integer>> sem = LiveStatus.getSEM();
                Word tem1 = LiveStatus.getSEM().peek().getFirst();
                LiveStatus.getSEM().pop();
                Word tem2 = LiveStatus.getSEM().peek().getFirst();
                LiveStatus.getSEM().pop();
                Quaternary tem = new Quaternary();
                tem.setOperator(new Word("="));
                tem.setArg1(tem1);
...
//                    Analysis.flag = false;
            ...
            case Constants.GREA: { // GREA动作
                ...
                tem.setArg1(str2);
                tem.setArg2(str1);
                tem.setResult(new Word(String.valueOf(LiveStatus.getQt().size() + 2)));
                if (LiveStatus.getSIGNHEAD().size() != 0) {
                    String[] strings = LiveStatus.getSIGNHEAD().peek();
                    if (strings[0].equals("dow") && strings[1].equals(String.valueOf(thelevel))) {
                        tem.setResult(new Word(String.valueOf(strings[2])));
                        LiveStatus.getSIGNHEAD().pop();
                    }
                }
                if (!isDefined(tem.getArg1(), table)) {
                    Analysis.flag = false;
            ...
            case Constants.EL: { // EL动作
                ...
                tem.setLevel(thelevel);
                LiveStatus.getQt().add(tem);
                for (int i = LiveStatus.getQt().size() - 1; i >= 0; i--) {
                    if (LiveStatus.getQt().get(i).getOperator().equals(new Word("if")) && LiveStatus.getQt().get(i).getResult().equals(new Word("_")) && LiveStatus.getQt().get(i).getLevel() == thelevel) {
                        Quaternary tem1 = new Quaternary();
                        tem1.setOperator(new Word("j"));
                        tem1.setArg1(new Word("_"));
                        tem1.setArg2(new Word("_"));
                        tem1.setResult(new Word(String.valueOf(LiveStatus.getQt().size())));
                        LiveStatus.getQt().set(i, tem1);
                        break;
                    }
                }
                break;
            ...
            case Constants.IESEC: { // IESEC动作
                for (int i = LiveStatus.getQt().size() - 1; i >= 0; i--) {
                    if (LiveStatus.getQt().get(i).getOperator().equals(new Word("el")) && LiveStatus.getQt().get(i).getResult().equals(new Word("_")) && LiveStatus.getQt().get(i).getLevel() == thelevel) {
                        Quaternary tem1 = new Quaternary();
                        tem1.setOperator(new Word("j"));
                        tem1.setArg1(new Word("_"));
                        tem1.setArg2(new Word("_"));
            ...
            case Constants.WE: { // WE动作
                ...
                for (int i = LiveStatus.getQt().size() - 1; i >= 0; i--) {
                    if (LiveStatus.getQt().get(i).getOperator().equals(new Word("do")) && LiveStatus.getQt().get(i).getResult().equals(new Word("_")) && isFindDo == 0) {
                        isFindDo = 1;
                        Quaternary tem1 = new Quaternary();
                        tem1.setOperator(new Word("j"));
                        tem1.setArg1(new Word("_"));
                        tem1.setArg2(new Word("_"));
                        tem1.setResult(new Word(String.valueOf(LiveStatus.getQt().size() + 1)));
                        LiveStatus.getQt().set(i, tem1);
                    }
                    if (isFindDo == 1 && LiveStatus.getQt().get(i).getOperator().equals(new Word("j"))) {
                        Quaternary tem1 = new Quaternary();
                        tem1.setOperator(new Word("j"));
                        tem1.setArg1(new Word("_"));
                        tem1.setArg2(new Word("_"));
                        tem1.setResult(new Word(String.valueOf(i - 1)));
                        LiveStatus.getQt().add(tem1);
                        break;
                    }
                }
                break;
            ...
            case Constants.DOW: { // DO_While动作
                LiveStatus.getSIGNHEAD().add(new String[]{"dow", String.valueOf(thelevel), String.valueOf(LiveStatus.getQt().size())});
                break;
            }
            ...
            case Constants.ADDFUN: {
                ...
                Stack<Pair<Word, Integer>> sem = LiveStatus.getSEM();
                SymbolTable symbolTable = new SymbolTable(Analysis.nowFunc, LiveStatus.getSEM().pop().getFirst().getWord());
                ...
                if (word.getWord().equals("main")) {
                    ...
                    q.setArg2(new Word("_"));
                    q.setResult(new Word("_"));
                    FinalAttribute.getSymbolTable(word.getWord()).getLiveStatus().getQt().add(q);
                }
            }
            break;
            ...
            case Constants.RET: {
                Word str = LiveStatus.getSEM().peek().getFirst();
                LiveStatus.getSEM().pop();
                for (int i = LiveStatus.getQt().size() - 1; i >= 0; i--) {
                    if (LiveStatus.getQt().get(i).getOperator().equals(new Word("ret")) && LiveStatus.getQt().get(i).getResult().equals(new Word("_")) && LiveStatus.getQt().get(i).getLevel() == thelevel) {
                        Quaternary tem1 = new Quaternary();
            ...
            case Constants.CALLS: {
                ...
                while (!LiveStatus.getArgsStack().empty()) {
                    list.add(0, LiveStatus.getArgsStack().pop().getFirst());
                }
                for (int i = LiveStatus.getQt().size() - 1; i >= 0; i--) {
                    if (LiveStatus.getQt().get(i).getOperator().equals(new Word("call")) && LiveStatus.getQt().get(i).getResult().equals(new Word("_")) && LiveStatus.getQt().get(i).getLevel() == thelevel) {
                        LiveStatus.getQt().get(i).setArg2(new Word(list.toString()));
                    }
                }
            }
            break;
            ...
            case Constants.PRINT: {
                Word word1 = LiveStatus.getSEM().peek().getFirst();
                ...
                LiveStatus.getQt().add(quaternary);
            }
            break;
            ...
            case Constants.INDE: {
                Word word1 = LiveStatus.getSEM().peek().getFirst();
                LiveStatus.getSEM().pop();
                Word word2 = LiveStatus.getSEM().peek().getFirst();
                LiveStatus.getSEM().pop();
                Quaternary quaternary = new Quaternary();
    ...
    private static boolean isDefined(Word name, SymbolTable table) {
        if (isNumber(name)) {
            ...
            String[] ss = s[1].split("\\]");
            for (SymbolTable.Var v : table.SynTable) {
    ...
    public static void getTemLable(Quaternary tem, SymbolTable table) {
        ...
        if (tem.getOperator().equals(new Word("+")) || tem.getOperator().equals(new Word("-")) || tem.getOperator().equals(new Word("*")) || tem.getOperator().equals(new Word("/"))) {
            ...
            if (!newtemp.type.equals("")) {
                ...
                newtemp.offset = table.SynTable.get(table.SynTable.size() - 1).offset + size;
                newtemp.scope = new ArrayList<>(Analysis.actionScope);
                newtemp.isTemp = true;
                table.SynTable.add(newtemp);
            }
        }
    }
...
//    public static void printQuaternary(LiveStatus liveStatu) {
